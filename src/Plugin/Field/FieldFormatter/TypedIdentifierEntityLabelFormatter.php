...
/**
 * Formatter that displays matched entity title as a link.
 *
 * This formatter finds entities that share matching typed identifiers with the
 * current entity and displays their title as a link.
 *
 * @FieldFormatter(
 *   id = "typed_identifier_entity_label",
 *   label = @Translation("Entity Label/Link (via typed identifier)"),
 *   field_types = {
 *     "typed_identifier"
 *   }
 * )
 */
class TypedIdentifierEntityLabelFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs a TypedIdentifierEntityLabelFormatter object.
   *
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $element['link_label'] = [
      ...
      '#description' => $this->t('If enabled, wrap the entity label with a link to the entity.'),
  ...
  public function settingsSummary() {
    ...
    if ($this->getSetting('link_label')) {
      $summary[] = $this->t('Entity label linked');
    }
    ...
    foreach ($items as $delta => $item) {
      ...
      try {
        ...
        if ($matched_entity and !array_key_exists($matched_entity->id(), $uniqkeys)) {
          ...
          if ($this->getSetting('link_label')) {
            // Build link to the matched entity.
            $elements[$delta] = [
              '#type' => 'link',
              '#title' => $matched_entity->label(),
              '#url' => $matched_entity->toUrl(),
              '#cache' => [
                'tags' => [
            ...
            $elements[$delta] = [
              '#plain_text' => $matched_entity->label(),
              '#cache' => [
                'tags' => [
                  'node:' . $matched_entity->id(),
                  'field_config:node.' . $target_bundle . '.' . $target_field,
